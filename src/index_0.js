...
function testSnapshotAnnotation(){
	const urlParams = new URLSearchParams(window.location.search);
        ...
        const anno = new LayerAnnotation(aOptions);
        lime.addLayer('anno', anno);

        // If editorEnable, create a SVG annotation Editor
        if (editorEnable) {
            const editor = new EditorSvgAnnotation(lime, anno, {
                classes: classParam
            });
        ...
        button.addEventListener('click', (event) => {
            ...
            e.setAttribute('href', ui.viewer.canvas.canvasElement.toDataURL());
...
function lensTest() {
	...
	lime.camera.bounded = false;
	const controllerLens = new ControllerLens({
		lensLayer: lensLayer,
		camera: lime.camera,
		hover: true,
		priority: 0
